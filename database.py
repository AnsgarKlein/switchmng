...
def query_port_type(description):
...
def modify_switch_model(name, **kwargs):
    # Check switch model
    sm = query_switch_model(name)
    if sm is None:
        return None
...
def add_switch_model(name, ports, size = None):
    # Check all arguments before making any changes
    if type(name) is not str:
        return None

    if query_switch_model(name) is not None:
        return None

    if ports is not None:
        try:
            ports = _port_model_maps_to_dict(ports)
        except:
            return None
...
def modify_switch(name, **kwargs):
    # Check switch
    sw = query_switch(name)
    if sw is None:
        return None
...
def add_switch(name, model, port_maps = None, location = None):
    # Check all arguments before making any changes
    if type(name) is not str:
        return None

    if query_switch(name) is not None:
        return None

    if type(model) is not str:
        return None
    model = query_switch_model(model)
    if model is None:
        return None

    if port_maps is not None:
        try:
            port_maps = _port_maps_to_dict(port_maps)
        except:
            return None
...
def modify_port_type(description, **kwargs):
    # Check port type
    pt = query_port_type(description)
    if pt is None:
        return None
...
def add_port_type(description = None, speed = None):
    pt = PortType(description = description, speed = speed)
    session.add(pt)
    session.commit()
    return pt

def modify_vlan(tag, description = None):
    vl = query_vlan(tag)
    if not vl:
        return None
...
def add_vlan(tag = None, description = None):
    vl = Vlan(tag = tag, description = description)
    session.add(vl)
    session.commit()
    return vl

def _port_model_maps_to_dict(port_maps):
    ...
    for port_map in port_maps:
        ...
        name = port_map['name']
        port_type = query_port_type(port_map['port_type'])
...
def _port_maps_to_dict(port_maps):
    if type(port_maps) is not list:
        raise TypeError('Given list of port maps is not a list')
    ...
    for port_map in port_maps:
        ...
        name = port_map['name']
        vlans = [ query_vlan(v) for v in port_map['vlans'] ]
